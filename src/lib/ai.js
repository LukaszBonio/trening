...
export async function generateAIPlan({ type, goal, equipment = 'siłownia', avoid = '', signal }) {
...
Cel: ${goalDesc}
Struktura: ${struct}${avoidLine}

Zwróć WYŁĄCZNIE poprawny JSON w formacie (bez markdown, bez komentarzy):
{
  "name": "krótka nazwa planu (max 40 znaków, po polsku)",
  "exercises": [
    { "name": "nazwa ćwiczenia po polsku", "sets": LICZBA, "reps": "ZAKRES jako string np. '8-10'", "tip": "krótka wskazówka techniczna (max 60 znaków)" }
  ]
}
...
- Trzymaj się struktury i liczby ćwiczeń podanej wyżej.`
