...
Aplikacja zbudowana jest na **Vue 3 + Vite + Pinia** — 20 komponentów, 5 composables, 9 stores,
19 lib modules z 138 testami (Vitest). PWA z Service Workerem zapewnia działanie offline.
...
**Testy:** `npm test` — 138 testów Vitest (composables, lib modules, stores). Env `node` (bez jsdom).
...
├── test/                       # 13 plików testowych Vitest (138 testów)
...
| **lib/muscles.js** | Słownik 25 głów mięśniowych, detectMuscle, detectEquipment |
| **lib/plans.js** | 48 gotowych planów (10 typów), getRandomPlan |
| **lib/substitutions.js** | findSubstitutes, youtubeSearchUrl |
...
| **lib/weeklyReport.js** | Tygodniowy raport AI z cache (14 dni historii → highlights + sugestie) |
...
| **Testy** | [Vitest 3.2](https://vitest.dev/) — 138 testów (13 plików), env `node` |
...
- [x] 9 Pinia stores, 20 komponentów, 5 composables, 5 widoków z lazy-loadingiem
- [x] Vue Router (hash mode), page transitions
- [x] 138 testów Vitest (13 plików) — composables, lib modules, stores
...
### 🚧 V3 — *następne kroki*
- [ ] TypeScript migration (start od `lib/`)
