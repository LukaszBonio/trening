...
describe('buildCoachChatSystem', () => {
  ...
  it('styl "motywujacy" zmienia ton promptu', () => {
    const p = buildCoachChatSystem({ goalLabel: 'Masa', history, style: 'motywujacy' })
    expect(p.toLowerCase()).toContain('motywując')
  })
})
...
describe('executeCoachTool', () => {
  ...
  it('COACH_TOOLS ma 9 narzędzi z poprawnymi nazwami', () => {
    expect(COACH_TOOLS.map(t => t.name)).toEqual([
      'lista_cwiczen', 'progres_cwiczenia', 'analiza_wzorcow',
      'sugestia_ciezaru', 'rekordy_osobiste', 'podsumowanie',
      'technika_cwiczenia', 'dziennik_wagi', 'wolumen_partii'
    ])
  })
})
